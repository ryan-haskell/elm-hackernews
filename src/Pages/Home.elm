...
import HackerNews exposing (HackerNews, Story)
import Time
import Types
...
init : ( Model, Cmd Msg )
init =
    Model [] ! [ HackerNews.fetch HackerNews.topStories InitializeTopStories ]
...
view context model =
    column
        ...
        [ viewNavbar
        , container <| viewStories (HackerNews.items model.topStories)
        , container <|
            el
                [ padding 24
                , width fill
            ...
            <|
                if HackerNews.hasMore model.topStories then
                    Input.button
                        [ centerX
                        , paddingXY 24 12
                        , Border.rounded 4
